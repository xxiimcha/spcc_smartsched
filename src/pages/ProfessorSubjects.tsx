  ...
  const ProfessorSubjects: React.FC = () => {
    ...
    const [strand, setStrand] = useState<string>("all"); // store exact label (e.g., "GAS", "HUMSS")

    // data
    const [subjects, setSubjects] = useState<SubjectDTO[]>([]);
    const [prefs, setPrefs] = useState<PrefMap>({});
    const [willing, setWilling] = useState<WillingMap>({}); // persisted now
    ...
    // derive strands from the current payload (used as fallback if allStrands is empty)
    ...
    // fetch subjects + existing prefs (once)
    useEffect(() => {
      let alive = true;
      const timer = setTimeout(async () => {
        try {
          ...
          const res = await apiService.getSubjects({
            ...
            // ✅ pass the exact strand label to backend (no lower-casing)
          ...
          // ✅ capture ALL strands only when viewing "All"
          if (strand === "all") {
            const s = new Set<string>();
            list.forEach((x: any) => {
              const v = (x?.strand ?? "").toString().trim();
    ...
    const displayed = useMemo(() => {
      ...
      return subjects.filter((s) => {
        ...
        // ✅ case-insensitive compare, but we keep the state value exact
        const byStrand =
          strand === "all" ||
    ...
    // Build a live summary array from selections
    ...
    // Save — enforces willingness on every selected subject
    const handleSave = async () => {
      // Block when any willingness is missing
      ...
      const selections = Object.entries(prefs)
        ...
        .map(([k, v]) => {
          ...
          return {
            ...
            willingness: willing[id]!, // safe due to guard
          };
        });
    ...
    // ✅ tabs should come from a stable "all strands" list
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[260px_minmax(0,1fr)_320px] gap-6">
          ...
          <div className="min-h-[300px]">
            {/* ✅ Strand Tabs (All + stable list of strands) */}
          ...
          {/* LIVE Selection Summary (updates on add/remove) */}
          <Card className="h-min xl:sticky xl:top-4">
            <CardHeader className="pb-2">
